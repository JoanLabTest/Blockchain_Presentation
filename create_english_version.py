...
# -*- coding: utf-8 -*-
"""
Script pour créer index-simple_en.html en traduisant tout le contenu
"""
...
def main():
    ...
    print("🔧 Traduction du contenu en anglais...")
    
    # Changer la langue HTML
    content = re.sub(r'<html lang="fr">', '<html lang="en">', content)
    
    # Changer le titre
    content = re.sub(
        r'<title>DCM Digital - Portail Stratégique</title>',
        '<title>DCM Digital - Strategic Portal</title>',
        content
    )
    
    # Modifier le sélecteur de langue dans la navigation
    # Remplacer les boutons JavaScript par des liens
    lang_switcher_old = r'''        <!-- LANGUAGE SWITCHER -->
    ...
    lang_switcher_new = '''        <!-- LANGUAGE SWITCHER -->
        <div style="display: flex; align-items: center; gap: 20px;">
            <div class="lang-switch">
    ...
    content = content.replace(lang_switcher_old, lang_switcher_new)
    
    # Supprimer tout le système JavaScript de traduction (pas nécessaire)
    # Trouver et supprimer le bloc <script> avec les translations
    content = re.sub(
        r'    <!-- BILINGUAL TRANSLATION SYSTEM -->.*?</script>\s*\n',
        '',
        content,
        flags=re.DOTALL
    )
    
    # Maintenant traduire tout le contenu texte en utilisant les traductions du dictionnaire
    translations = {
        # Navigation
        "Accueil": "Home",
        "Historique": "History",
        ...
        "Au lieu de réconcilier deux fichiers internes (T+2), toutes les banques lisent le même registre en temps réel (T+0).": "Instead of reconciling two internal files (T+2), all banks read the same registry in real-time (T+0).",
        "ACCÈS": "ACCESS",
        ...
        # Roadmap
        "Plan d'Action : Lancer un Pilote Bancaire": "Action Plan: Pilot Launch",
        ...
        # Trader
        "Le Bureau du Trader 2026": "Trader Desk 2026",
        "Side": "Side",
        "Quantité": "Quantity",
    ...
    print("✅ Fichier index-simple_en.html créé avec succès!")
