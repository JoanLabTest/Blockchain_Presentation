...
const getHtml = (lang, paper) => {
    const isFr = lang === 'fr';
    const langCode = isFr ? 'fr' : 'en';
    const d = isFr ? paper.fr : paper.en;
    
    // Read the index to copy navbar/footer roughly, or just use a template snippet.
    // For simplicity, I'm composing a standard template based on the previous pages.
    const title = `${d.title} | DCM Core Institute`;
    const instTitle = isFr ? 'Institut' : 'Institute';
    const resNav = isFr ? 'Recherche' : 'Research';
    const dataNav = isFr ? 'Données' : 'Data';
    const regNav = isFr ? 'Réglementation' : 'Regulation';
    const diaNav = isFr ? 'Dialogue' : 'Dialogue';
    const comNav = isFr ? 'Communauté' : 'Community';
...
<head>
    ...
    <style>
        ...
        .paper-container { max-width: 800px; margin: 0 auto; padding: 60px 20px; background: #fff; color: #111; border-radius: 8px; margin-top: 40px; margin-bottom: 40px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); }
        ...
        .paper-abstract { font-style: italic; padding: 20px; background: #f9f9f9; border-left: 4px solid var(--inst-blue); margin-bottom: 40px; line-height: 1.6; }
...
<body>
    <nav class="navbar-pro">
        <a class="nav-brand" href="../../index.html">
            ...
            <div class="logo-text">DCM <span>Core ${instTitle}</span></div>
        </a>
        <div class="nav-actions">
            <div class="lang-switcher">
                <a href="../../${isFr ? 'en' : 'fr'}/research-papers/wp-${paper.id}.html" class="lang-link">${isFr ? 'EN' : 'FR'}</a>
            </div>
            <a href="index.html" class="auth-btn">${isFr ? 'Retour' : 'Back'}</a>
            <button class="connect-btn" onclick="window.print();"><i class="fas fa-print"></i> ${isFr ? 'Imprimer PDF' : 'Print PDF'}</button>
        </div>
    </nav>

    <div class="paper-container">
        <div class="paper-header">
            <span class="paper-number">${isFr ? paper.numFR : paper.numEN}</span>
        ...
        <div class="paper-abstract">
            <strong>${isFr ? 'Résumé' : 'Abstract'}:</strong><br/>
            ${d.abstract}<br/><br/>
...
papers.forEach(p => {
    fs.writeFileSync(path.join(__dirname, 'en/research-papers', \`wp-\${p.id}.html\`), getHtml('en', p));
    fs.writeFileSync(path.join(__dirname, 'fr/research-papers', \`wp-\${p.id}.html\`), getHtml('fr', p));
});
console.log('Papers generated.');
